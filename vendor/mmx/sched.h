...
ABOUT:
    ...
    conversion of Doug Binks enkiTS library
(https://github.com/dougbinks/enkiTS).

    Project Goals
    - ANSI C: Designed to be easy to embed into other languages
    - Embeddable: Designed as a single header library to be easy to embed into
your code.
    - Lightweight: Designed to be lean so you can use it anywhere easily, and
understand it.
...
thread which created the Task System.
    - Up-front Allocation friendly: Designed for zero allocations during
scheduling.
...
typedef void (*sched_run)(void *, struct scheduler *,
                          struct sched_task_partition, sched_uint thread_num);
struct sched_task {
    void *userdata;
...
SCHED_API void scheduler_init(struct scheduler *, sched_size *needed_memory,
                              sched_int thread_count,
                              const struct sched_profiling *);
/*  this function clears the scheduler and calculates the needed memory to run
    Input:
    -   number of os threads to create inside the scheduler (or SCHED_DEFAULT
...
*/
SCHED_API void scheduler_add(struct scheduler *, struct sched_task *,
...
typedef int
    sched__check_ptr_size[(sizeof(void *) == sizeof(SCHED_UINT_PTR)) ? 1 : -1];
...
template <typename T, int size_diff> struct sched_helper {
    enum { value = size_diff };
};
template <typename T> struct sched_helper<T, 0> {
...
#define SCHED_ALIGN_PTR(x, mask)                                               \
    (SCHED_UINT_TO_PTR(                                                        \
        (SCHED_PTR_TO_UINT((sched_byte *)(x) + (mask - 1)) & ~(mask - 1))))
...
SCHED_INTERN void sched_memset(void *ptr, sched_int c0, sched_size size) {
...
SCHED_INTERN void sched_zero_size(void *ptr, sched_size size) {
...
#else
#define SCHED_BASE_MEMORY_BARRIER_ACQUIRE()                                    \
    __asm__ __volatile__("" : : : "memory")
#define SCHED_BASE_MEMORY_BARRIER_RELEASE()                                    \
    __asm__ __volatile__("" : : : "memory")
#define SCHED_BASE_ALIGN(x) __attribute__((aligned(x)))
#endif

SCHED_INTERN sched_uint sched_atomic_cmp_swp(volatile sched_uint *dst,
                                             sched_uint swap, sched_uint cmp) {
/* Atomically performs: if (*dst == swapTp){ *dst = swapTo;}
 * return old *dst (so if sucessfull return cmp) */
#if defined(_WIN32) && !(defined(__MINGW32__) || defined(__MINGW64__))
    /* assumes two's complement - unsigned /signed conversion leads to same bit
     * pattern */
...
SCHED_INTERN sched_int sched_atomic_add(volatile sched_int *dst,
                                        sched_int value) {
...
SCHED_INTERN sched_int sched_thread_create(sched_thread *returnid,
                                           DWORD(WINAPI *StartFunc)(void *),
                                           void *arg) {
    DWORD thread;
...
SCHED_INTERN sched_int sched_thread_term(sched_thread threadid) {
    return CloseHandle(threadid) == 0;
}

SCHED_INTERN sched_uint sched_num_hw_threads(void) {
    SYSTEM_INFO sysinfo;
    GetSystemInfo(&sysinfo);
    return sysinfo.dwNumberOfProcessors;
}

SCHED_INTERN void sched_semaphore_create(struct sched_semaphore *s) {
    s->sem = CreateSemaphore(0, 0, MAXLONG, 0);
}

SCHED_INTERN void sched_semaphore_close(struct sched_semaphore *s) {
    CloseHandle(s->sem);
}

SCHED_INTERN void sched_semaphore_wait(struct sched_semaphore *s) {
    DWORD ret = WaitForSingleObject(s->sem, INFINITE);
    SCHED_ASSERT(ret != WAIT_FAILED);
}

SCHED_INTERN void sched_semaphore_signal(struct sched_semaphore *s, int cnt) {
    if (!cnt)
        return;
 ...
 * ---------------------------------------------------------------*/
/*  PIPE
    Single writer, multiple reader thread safe pipe using (semi) lockless
   programming Readers can only read from the back of the pipe The single writer
   can write to the front of the pipe, and read from both ends (a writer can be
   a reader) for many of the principles used here, see
...
/* utility function, not intended for general use. Should only be used very
 * prudenlty*/
#define sched_pipe_is_empty(p) (((p)->write - (p)->read_count) == 0)

SCHED_INTERN sched_int sched_pipe_read_back(struct sched_pipe *pipe,
                                            struct sched_subset_task *dst) {
    ...
    while (1) {
        ...
        previous = sched_atomic_cmp_swp(
            &pipe->flags[actual_read], SCHED_PIPE_INVALID, SCHED_PIPE_CAN_READ);
    ...
    /* we update the read index using an atomic add, ws we've only read one
     * piece of data. This ensures consitency of the read index, and the above
...
SCHED_INTERN sched_int sched_pipe_read_front(struct sched_pipe *pipe,
                                             struct sched_subset_task *dst) {
    sched_uint prev;
    ...
    while (1) {
        ...
        prev = sched_atomic_cmp_swp(&pipe->flags[actual_read],
                                    SCHED_PIPE_INVALID, SCHED_PIPE_CAN_READ);
...
SCHED_INTERN sched_int sched_pipe_write(struct sched_pipe *pipe,
                                        const struct sched_subset_task *src) {
    sched_uint actual_write;
    sched_uint write_index;
    SCHED_ASSERT(pipe);
    SCHED_ASSERT(src);

    /* The writer 'owns' the write index and readers can only reduce the amout
     * of data in the pipe. We get hold of both values for consistentcy and to
...
SCHED_GLOBAL const sched_size sched_pipe_align =
    SCHED_ALIGNOF(struct sched_pipe);
SCHED_GLOBAL const sched_size sched_arg_align =
    SCHED_ALIGNOF(struct sched_thread_args);
SCHED_GLOBAL const sched_size sched_thread_align = SCHED_ALIGNOF(sched_thread);
SCHED_GLOBAL const sched_size sched_semaphore_align =
    SCHED_ALIGNOF(struct sched_semaphore);
...
SCHED_INTERN void sched_wake_threads(struct scheduler *s) {
    sched_semaphore_signal(s->new_task_semaphore, s->thread_waiting);
}
SCHED_INTERN void sched_split_add_task(struct scheduler *s,
                                       sched_uint thread_num,
                                       struct sched_subset_task *st,
                                       sched_uint range_to_split,
                                       sched_int off) {
    sched_int cnt = 0;
...
SCHED_INTERN sched_int sched_try_running_task(struct scheduler *s,
                                              sched_uint thread_num,
                                              sched_uint *pipe_hint) {
    /* check for tasks */
    struct sched_subset_task subtask;
    sched_int have_task =
        sched_pipe_read_front(&s->pipes[thread_num], &subtask);
    ...
    while (!have_task && check_count < s->threads_num) {
        ...
        if (thread_to_check != thread_num)
            have_task =
                sched_pipe_read_back(&s->pipes[thread_to_check], &subtask);
        ++check_count;
    }
    if (have_task) {
        sched_uint part_size = subtask.partition.end - subtask.partition.start;
        /* update hint, will preserve value unless actually got task from
         * another thread */
        *pipe_hint = thread_to_check;
        if (subtask.task->range_to_run < part_size) {
            struct sched_subset_task t =
                sched_split_task(&subtask, subtask.task->range_to_run);
            sched_split_add_task(s, gtl_thread_num, &subtask,
                                 subtask.task->range_to_run, 0);
            subtask.task->exec(t.task->userdata, s, t.partition, thread_num);
            sched_atomic_add(&t.task->run_count, -1);
        } else {
            /* the task has already been divided up by scheduler_add, so just
             * run */
            subtask.task->exec(subtask.task->userdata, s, subtask.partition,
                               thread_num);
...
SCHED_INTERN void sched_call(sched_profiler_callback_f fn, void *usr,
                             sched_uint threadid) {
    if (fn)
        fn(usr, threadid);
}

SCHED_INTERN void scheduler_wait_for_work(struct scheduler *s,
                                          sched_uint thread_num) {
    sched_uint i = 0;
...
SCHED_INTERN SCHED_THREAD_FUNC_DECL sched_tasking_thread_f(void *pArgs) {
...
SCHED_API void scheduler_init(struct scheduler *s, sched_size *memory,
                              sched_int thread_count,
                              const struct sched_profiling *prof) {
    SCHED_ASSERT(s);
    SCHED_ASSERT(memory);

    sched_zero_struct(*s);
    s->threads_num = (thread_count == SCHED_DEFAULT) ? sched_num_hw_threads()
                                                     : (sched_uint)thread_count;

    /* ensure we have sufficent tasks to equally fill either all threads
     * including main or just the threads we've launched, this is outisde the
...
SCHED_API void scheduler_start(struct scheduler *s, void *memory) {
    sched_uint i = 0;
    SCHED_ASSERT(s);
    SCHED_ASSERT(memory);
    if (s->have_threads)
    ...
    s->args = (struct sched_thread_args *)SCHED_ALIGN_PTR(
        SCHED_PTR_ADD(void, s->threads, sizeof(sched_thread) * s->threads_num),
        sched_arg_align);
    s->new_task_semaphore = (struct sched_semaphore *)SCHED_ALIGN_PTR(
        s->args + s->threads_num, sched_semaphore_align);
...
SCHED_API void scheduler_add(struct scheduler *s, struct sched_task *task,
                             sched_run func, void *pArg, sched_uint size,
                             sched_uint min_range) {
...
SCHED_API void scheduler_join(struct scheduler *s, struct sched_task *task) {
...
SCHED_API void scheduler_wait(struct scheduler *s) {
...
SCHED_API void scheduler_stop(struct scheduler *s, int doWait) {
    sched_uint i = 0;
    SCHED_ASSERT(s);
    if (!s->have_threads)
        return;
